...
"""Render the canonical Smallstep CA security standard as a polished PDF."""
...
PAGE_BREAK_HEADINGS: Final[frozenset[str]] = frozenset(
    {"Installed layout", "Backups and recovery"}
)
...
def repository_root(script_path: Path) -> Path:
    """Return the repository root for a renderer inside ``smallstep-ca``."""
...
def build_pdf(blocks: Sequence[MarkdownBlock], output_path: Path) -> None:
    ...
    class SecurityDocument(BaseDocTemplate):
        ...
        def __init__(self, filename: str) -> None:
            super().__init__(
                ...
                title="Smallstep CA Security Standard",
                author="Michael Welter",
                subject="Client-neutral online intermediate CA baseline",
            )
            frame = Frame(
                self.leftMargin,
                self.bottomMargin,
        ...
        def draw_page(self, canvas: object, document: object) -> None:
            ...
            canvas.saveState()
            canvas.setTitle("Smallstep CA Security Standard")
            ...
            if page_number > 1:
                ...
                canvas.drawString(0.72 * inch, 10.53 * inch, "SMALLSTEP CA SECURITY STANDARD")
    ...
    try:
        ...
        cover = Table(
            [
                [
                    Paragraph("Smallstep CA<br/>Security Standard", styles["CoverTitle"]),
                ],
                [
                    Paragraph(
                        "Offline-root separation, constrained online issuance, "
                        "private listener hardening, and evidence-driven operations",
...
def build_argument_parser(default_source: Path, default_output: Path) -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Render the canonical Smallstep CA security standard as PDF."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments.check:
        print(f"Validated canonical Smallstep CA security standard: {source_path}")
        return 0
    build_pdf(blocks, output_path)
    print(f"Rendered Smallstep CA security standard PDF: {output_path}")
    return 0

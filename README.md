...
| `/bin/certbot` | Certbot binary used by `systemd/system/certbot.service`. Adjust the unit if needed. |
...
- `systemd/system/` contains the nginx, per-site PHP-FPM, and Certbot renewal units.
...
`systemd/system/certbot.timer` invokes `systemd/system/certbot.service`. The service always

...
| `/usr/sbin/nginx` | nginx binary used by `systemd/system/nginx.service`. Adjust the unit if the package installs it elsewhere. |
| `/bin/certbot` | Native Certbot binary used only by `systemd/system/certbot.service`; it must not resolve to the Snap launcher. |
| `/usr/bin/snap` and `snap.certbot.renew.service` | Snap command and snapd-generated renewal service used by the optional Snap Certbot drop-ins. |
| `/bin/systemctl` | systemctl binary used by the Certbot post-renewal reload. |
| `/sbin/php-fpm` | Administrator-managed, version-neutral PHP-FPM binary contract used by `systemd/system/php-fpm@.service`. |
...
- `systemd/system/` contains the nginx, per-site PHP-FPM, and Certbot renewal
  units plus optional Snap Certbot integration drop-ins.
...
`systemd/system/nginx.service` makes the host filesystem read-only to nginx except
...
The repository timer owns renewal scheduling for both supported backends:

| Contract | Native backend | Snap backend |
| --- | --- | --- |
...
Select exactly one backend and leave exactly one renewal scheduler active. The
Snap integration changes only the timer payload: it retains the repository's
calendar, randomized delay, and persistent catch-up policy while snapd runs
Certbot through its generated service. Snap-generated base units and their
revision-specific mount dependencies remain package-owned host state; never
copy or edit them in this repository.

Both payloads attempt an nginx reload after Certbot exits, including when
Certbot reports a partial renewal failure. This is intentional: certificates
...
The native Certbot service is confined to the webroot renewal flow used by
this repository. It can write only its systemd-managed configuration, state,
and log directories plus `/var/www/letsencrypt`; it cannot bind a listening
socket and has no Linux capabilities. Native renewal hooks and authenticator
plugins inherit this sandbox. A deployment using the standalone authenticator,
an HSM, or a hook that writes elsewhere must add the smallest necessary
systemd drop-in and re-run the dry-run validation.

Do not weaken the native service's capability, namespace, or home protections
to run the Snap launcher. `snap-confine` requires an execution environment
that deliberately conflicts with those restrictions. Select the optional
Snap payload instead; its service drop-in restores the nginx dependency,
one-hour timeout, restrictive umask, and audited reload without modifying
snapd's generated base service.

`SystemCallFilter` and `MemoryDenyWriteExecute` are intentionally not enabled
for the native Certbot service until they have been tested with the target
host's Python, cryptography library, and installed plugins.

Many distributions install their own Certbot timer. Disable the package-owned
scheduler when selecting this repository's timer, and repeat the one-scheduler
check after Certbot package or Snap refreshes.

Timer drop-ins survive base-unit reinstalls. When changing backends, stop
`certbot.timer` first and remove or park the previous backend's exact drop-ins;
installing `certbot.timer` again does not clear
`certbot.timer.d/10-snap-runner.conf`.
